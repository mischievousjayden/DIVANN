...
class DIVANN:
    def __init__(self, num_features, num_hidden, num_classes=1, beta=1.0):
        ...
        self._hidden = {
            "weight": tf.Variable(tf.random_normal([num_hidden, num_hidden//2]), name="hidden_weight"),
            "bias": tf.Variable(tf.random_normal([num_hidden//2]), name="hidden_bias")
        }

        self._decoder = {
            "weights": tf.Variable(tf.random_normal([num_classes, num_hidden//2, num_features]), name="decoder_weights"),
        ...
        self._histogram.append(tf.histogram_summary("hidden_weight", self._hidden["weight"]))
        self._histogram.append(tf.histogram_summary("hidden_bias", self._hidden["bias"]))
    ...
    def run(self, x):
        with tf.name_scope("encode") as scope:
            ...
            # self._encode = tf.nn.relu(tf.add(tf.matmul(x, self._encoder["weight"]), self._encoder["bias"]))

            self._encode2 = tf.nn.sigmoid(tf.add(tf.matmul(self._encode, self._hidden["weight"]), self._hidden["bias"]))

        with tf.name_scope("decode") as scope:
            self._decode = list()
            for i in range(self._num_classes):
                self._decode.append(tf.nn.sigmoid(tf.add(tf.matmul(self._encode2, self._decoder["weights"][i]), self._decoder["biases"][i])))
